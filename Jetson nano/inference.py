...
with graph.as_default():
    with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.50))) as sess:
        ...
        for i in range(n_time_inference):
            ...
            if out_pred[0][0] <= out_pred[0][1]: 
                print("inference-" + str(i)  + " " + x_path[i] + " predict: Covid-19" )
            else: 
                print("inference-" + str(i)  + " " + x_path[i] + " predict: Not Covid-19" )
        ...
        print("ACC : " + str(correct/len(x_path)) )
